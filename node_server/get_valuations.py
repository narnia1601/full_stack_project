...
class GetValuations:
    ...
    def get_52_week_low(self):
        self.annual_low_price = self.result['Technicals']['52WeekLow']
    ...
    def get_revenue_estimate_growth(self):
        ...
        if next_year_earnings_value == None:
            next_year_earnings_value = 0
    ...
    def get_cost_of_equity(self):
        ...
        self.cost_of_equity = risk_free_rate + (beta * market_risk_premium)
    ...
    def get_margin_of_safety(self):
        ...
        try:
            self.cash = list(self.result['Financials']['Balance_Sheet']['yearly'].values())[0]['cash']
            self.cash = self.get_value(self.cash)
        except:
            try:
                self.cash = list(self.result['Financials']['Balance_Sheet']['quarterly'].values())[0]['cash']
                self.cash = self.get_value(self.cash)
            except:
                self.cash = 0
        market_cap = self.result['Highlights']['MarketCapitalization']
        equity_value = enterprise_value - self.total_debt + self.cash
        self.shares_outstanding = self.result['SharesStats']['SharesOutstanding']
        try:
            price_per_share = market_cap / self.shares_outstanding
            self.price = price_per_share
            value_per_share = equity_value / self.shares_outstanding
            self.value = value_per_share
